...
export async function getShopDomain(): Promise<string> {
  try {
    const data = await storefrontApiRequest(GET_SHOP_QUERY);
    if (!data) return `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}`;
    return data.data.shop.primaryDomain?.url || `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}`;
  } catch (error) {
...
export function getPolicyUrl(policyType: 'terms-of-service' | 'privacy-policy' | 'refund-policy' | 'shipping-policy'): string {
  return `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/policies/${policyType}`;
}
